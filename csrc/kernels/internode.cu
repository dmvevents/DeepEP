/**
 * @file internode.cu
 * @brief Inter-node communication implementation for DeepEP with EFA/NVSHMEM
 * @details This file implements the core inter-node communication primitives for
 *          distributed MoE (Mixture of Experts) training using EFA transport layer
 *          instead of the original IBGDA implementation.
 * 
 * @note Major migration: IBGDA → EFA/NVSHMEM for improved AWS compatibility
 * @warning Performance critical: Changes affect distributed training throughput
 */
...
#include "launch.cuh"
#include "utils.cuh"
// #include "ibgda_device.cuh"  // Replaced with EFA implementation
#include "nvshmem_device.cuh"
// #include "efa_device.cuh"   // Alternative EFA implementation


namespace deep_ep {

/**
 * @brief Notification dispatch kernel for MoE token distribution
 * @tparam kLowLatencyMode Enable low-latency optimizations for small messages
 * @tparam kNumRDMARanks Number of RDMA ranks in the cluster
 * @param num_tokens_per_rank Array of token counts per rank
 * @param moe_recv_counter_mapped Mapped receive counter for synchronization
 * @param rank Current processing element rank
 * 
 * @details This kernel orchestrates the initial phase of MoE communication,
 *          notifying remote nodes about incoming token distributions.
 * 
 * @note EFA Migration: Simplified synchronization model using nvshmem_quiet()
 */
...
/**
 * @brief Main dispatch kernel for inter-node MoE communication
 * @tparam kLowLatencyMode Enable optimizations for low-latency scenarios
 * @tparam kNumRDMARanks Number of RDMA ranks
 * @tparam kNumNVLRanks Number of NVLink ranks per RDMA rank
 * @param recv_x Received token data buffer
 * @param recv_x_scales Received scaling factors
 * @param recv_topk_idx Received top-k indices
 * @param recv_topk_weights Received top-k weights
 * @param send_x Send token data buffer
 * @param send_x_scales Send scaling factors  
 * @param send_topk_idx Send top-k indices
 * @param send_topk_weights Send top-k weights
 * @param moe_send_counter_mapped Send counter for synchronization
 * @param rank Current processing element rank
 * @param slot_idx Current slot index for double buffering
 * 
 * @details Core kernel implementing the distributed MoE token exchange pattern.
 *          Features warp-specialized roles and optimized memory transfers.
 * 
 * @note Performance Critical: This kernel determines distributed training throughput
 * @warning EFA Migration: Removed IBGDA-specific assertions and QP management
 */
...
dispatch(int4* recv_x, float* recv_x_scales, int64_t* recv_topk_idx, float* recv_topk_weights, SourceMeta* recv_src_meta,
    ...
    /**
     * @note EFA Migration: Removed IBGDA-specific assertions
     * @details Original assertion checked QP availability, no longer needed with NVSHMEM
     */
    ...
    if (warp_role == WarpRole::kRDMASender) {
        ...
        for (int dst_rdma_rank = warp_id; dst_rdma_rank < kNumRDMARanks; dst_rdma_rank += kNumDispatchRDMASenderWarps) {
            ...
            if (dst_rdma_rank != rdma_rank) {
                /**
                 * @brief EFA-optimized PUT operation with warp-level parallelization
                 * @details Uses the EFA-compatible PUT implementation from efa_device.cuh
                 * @note Performance: Maintains warp-level optimization for bandwidth
                 */
        ...
        /**
         * @brief Add fence after RDMA operations
         * @details Ensures proper ordering of metadata transfers
         * @note EFA Addition: Critical for correctness with EFA transport
         */
        nvshmem_fence();
...
/**
 * @brief Cached notification kernel with EFA optimizations
 * @tparam kLowLatencyMode Low-latency mode flag
 * @param rdma_clean_offset Offset for RDMA buffer cleaning
 * @param rdma_num_in_groups Number of input groups for RDMA
 * @param rdma_num_out_groups Number of output groups for RDMA
 * @param nvl_clean_offset Offset for NVLink buffer cleaning  
 * @param nvl_num_in_groups Number of input groups for NVLink
 * @param nvl_num_out_groups Number of output groups for NVLink
 * @param rank Current processing element rank
 * @param num_rdma_ranks Total number of RDMA ranks
 * 
 * @details Optimized notification kernel that caches frequently accessed data
 *          and provides more efficient buffer management for repeated operations.
 * 
 * @note EFA Migration: Simplified quiet operations using global NVSHMEM primitives
 */
...
__global__ void cached_notify(const int rdma_clean_offset, const int rdma_num_int_clean,
    ...
    if (sm_id == 0) {
        /**
         * @note EFA Migration: Replaced per-QP quiet with simplified approach
         * @details Original IBGDA required tracking individual queue pairs,
         *          EFA uses global quiet for all outstanding operations
         */
        // auto qps_per_rdma_rank = ibgda_get_state()->num_rc_per_pe * ibgda_get_state()->num_devices_initialized;
        ...
        /**
         * @brief EFA-optimized quiet operation for each destination rank
         * @details Ensures completion of operations to each remote rank
         * @note Performance: Reduces quiet calls while maintaining correctness
         */
        for (int i = thread_id; i < (num_rdma_ranks - 1); i += num_threads) {
            auto dst_rdma_rank = (i + rdma_rank + 1) % num_rdma_ranks;
            /**
             * @note EFA Implementation: One quiet call per destination rank
             * @details Balances performance and correctness requirements
             */
            // Call quiet for each target rank to ensure completion
...
/**
 * @brief Combine kernel for MoE result aggregation with EFA support
 * @tparam kLowLatencyMode Low-latency optimization flag
 * @tparam kNumRDMARanks Number of RDMA ranks in cluster
 * @tparam kNumNVLRanks Number of NVLink ranks per RDMA rank
 * @param combined_x Output combined token data
 * @param combined_topk_weights Output combined weights
 * @param combined_topk_idx Output combined indices
 * @param recv_x Input received token data
 * @param recv_topk_weights Input received weights
 * @param recv_topk_idx Input received indices
 * @param moe_recv_counter_mapped Receive counter for synchronization
 * @param rank Current processing element rank
 * @param slot_idx Slot index for double buffering
 * 
 * @details Aggregates results from distributed MoE computation across all nodes,
 *          combining tokens that were processed by different expert nodes.
 * 
 * @note EFA Migration: Added explicit fencing for data transfer reliability
 */
template<bool kLowLatencyMode,
        ...
        if (warp_role == WarpRole::kNVLAndRDMAForwarder) {
            ...
            for (int token_start_idx = 0; token_start_idx < num_tokens_to_combine; token_start_idx += num_max_rdma_chunked_send_tokens) {
                ...
                if (sub_warp_id == kNumWarpsPerForwarder - 1) {
                    ...
                    if (lane_id == 0) {
                        /**
                         * @brief Atomic tail update with EFA compatibility
                         * @details Uses the EFA-compatible atomic add implementation
                         */
...
#define COMBINE_LAUNCH_CASE(num_rdma_ranks) { \
    auto combine_func = low_latency_mode ? \
        combine<true, num_rdma_ranks, nv_bfloat16, kNumCombineForwarderWarps, kNumTMABytesPerSenderWarp, kNumTMABytesPerForwarderWarp> : combine<false, num_rdma_ranks, nv_bfloat16, kNumCombineForwarderWarps, kNumTMABytesPerSenderWarp, kNumTMABytesPerForwarderWarp>; \
